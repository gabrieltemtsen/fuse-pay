  ...
  export default function Home() {
    ...
    // State for chatbot moved here
    const [chat, setChat] = useState([]);
    const [userInput, setUserInput] = useState('');
    // In your component
    const [isEmployeeOfMonth, setIsEmployeeOfMonth] = useState(false);
    const [showGenerateNFTButton, setShowGenerateNFTButton] = useState(false);
  ...
  // 0x763b9295081c8e04F444F44A1cF14D7379F579c2
    ...
    // Function to handle chat input submission moved here
    const handleChatSubmit = async () => {
      ...
      if (prompt) {
        try {
          ...
          if (data.response) {
            setChat([...chat, { type: 'user', text: prompt }, { type: 'bot', text: data.response }]);
          } else {
  ...
  const { isConnected } = useAccount(); // This assumes you're using wagmi's useAccount hook which provides an isConnected boolean.

  useEffect(() => {
    // Assuming you have a method to determine if the connected user is the Employee of the Month
    if (isConnected && isEmployeeOfMonth) {
      setShowGenerateNFTButton(true);
    } else {
      setShowGenerateNFTButton(false);
    }
  }, [isConnected, isEmployeeOfMonth]);
  ...
  const handleGenerateNFT = async () => {
    ...
    return (
      <>
        <Layout chat={chat} setChat={setChat} userInput={userInput} setUserInput={setUserInput} handleChatSubmit={handleChatSubmit}>
          {showGenerateNFTButton && (
          <Button onClick={handleGenerateNFT} className="fixed bottom-10 right-10 bg-blue-600 text-white font-bold py-2 px-4 rounded">
            Generate NFT
          </Button>
        )}
